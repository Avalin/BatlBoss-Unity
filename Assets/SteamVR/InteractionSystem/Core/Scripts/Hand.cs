    ...
    public class Hand : MonoBehaviour
    {
        ...
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            if (attachedObject.HasAttachFlag(AttachmentFlags.SnapOnAttach))
            {
                ...
                else
                {
                    ...
                    //Sets rotation for different weapon types
                    if (objectToAttach.tag == "Torch")
                    {
                        if (handType == SteamVR_Input_Sources.RightHand)
                        {
                            objectToAttach.transform.Rotate(0, -45, 0);
                        }
                    ...
                    if (objectToAttach.tag == "Sword")
                    {
                        if (handType == SteamVR_Input_Sources.RightHand)
                        {
                            objectToAttach.transform.localPosition = new Vector3(0f, 0f, 0f);
                            objectToAttach.transform.localPosition += new Vector3(0.035f, 0.007f, 0.167f);
                            objectToAttach.transform.localRotation = Quaternion.Euler(184, -84, 91);
                        }
                        if (handType == SteamVR_Input_Sources.LeftHand)
                        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    attachedObject.initialRotationalOffset.x = 0;
                }
            }
        ...
        protected virtual void Update()
        {
            ...
            if (attachedObject != null)
            {
                ...
                if (SteamVR_Input._default.inActions.DropItem.GetStateDown(SteamVR_Input_Sources.Any))
                {
                    //Astrid: This is the detach button and functionality
